  ...
  */
  ...
  useEffect(() => {
    const syncRouteFromLocation = () => {
      ...
      const c = p.match(/^\/course\/(\d+)$/);
      if (m) {
        setRoute({ name: 'note', id: Number(m[1]) });
      } else if (c) { 
        setRoute({ name: 'course', id: Number(c[1]) });
      } else if (p === '/dashboard') {
  ...
  //Opens professor dashboard
  const openDashboard = () => {
    window.history.pushState({ route: 'dashboard' }, '', '/professor');
    setRoute({ name: 'dashboard', id: null });
    setSearchActive(false);
  };

  //Opens a course detail
  const openCourse = (id) => {
    window.history.pushState({ route: 'course', id }, '', `/course/${id}`);
    setRoute({ name: 'course', id });
    setSearchActive(false);
  };
    ...
    <div className="app-shell">
      ...
      <main className="main">
        {searchActive ? (
          ...
          <section style={{width: '100%'}}>
             <ProfessorDashboard onCourseSelect={openCourse} />
          </section>
        ) : route.name === 'course' ? (
          <section style={{width: '100%'}}>
            <CourseDetail id={route.id} onBack={openDashboard} /> 
          </section>
        ) : route.name === 'list' ? (
          <React.Fragment>
